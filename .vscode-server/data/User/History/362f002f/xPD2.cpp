    ...
    /* found repeated value */
        ...
        *&(*parentNode)->max = *&(*parentNode)->max > valueToInsert ? *&(*parentNode)->max : valueToInsert;
        ...
        if( *&(*parentNode)->m_subtrees[clusterNum]->min == NIL){
            auto depth = Insertion(&(*parentNode)->m_summary, clusterNum, counter+1); //we don't update the depth for summary!
            if(depth==0){return 0;}        
        }
        /* if the above condition is true, insertion is directly in the next cluster so o(1), else it'll be the only recursive call */
        auto depth =Insertion(&(*parentNode)->m_subtrees[clusterNum], clusterPos, (counter > 0?counter+1:(++m_depth,0))); // make sure that only after updating the summary we increase counter
        if(depth==0){return 0;}
        return m_depth;
    }
}
