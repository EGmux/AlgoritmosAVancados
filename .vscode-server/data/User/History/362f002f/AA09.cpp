...
uint32_t VanEndeBoas::Insertion(VEBTree** parentNode, uint32_t valueToInsert, uint32_t counter){
    ...
    /* found repeated value */
        ...
        *&(*parentNode)->max = *&(*parentNode)->max > valueToInsert ? *&(*parentNode)->max : valueToInsert;
        ...
        auto depth =Insertion(&(*parentNode)->m_subtrees[clusterNum], clusterPos, (counter > 0?counter++:counter)); // make sure that only after updating the summary we increase counter
        if(depth==0){return 0;}
        return m_depth;
    }
}
