...
uint32_t VanEndeBoas::Insertion(VEBTree** parentNode, uint32_t valueToInsert, uint32_t counter){
    ...
    /* found repeated value */
        ...
        *&(*parentNode)->max = *&(*parentNode)->max > valueToInsert ? *&(*parentNode)->max : valueToInsert;
        ...
        if(counter==1) m_depth++; // suppose insertion happened as described above, this nullify the depth increment, else it counts properly
        Insertion(&(*parentNode)->m_subtrees[clusterNum], clusterPos, ++counter);
        return m_depth;
    }
}
